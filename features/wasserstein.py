...
def sinkhorn_divergence(PC_pair, neighbor_mask_0, neighbor_mask_1, params_sinkhorn):
    ...
    masked_tensor0 = torch.where(
        neighbor_mask_0, neighbor_mask_0.to(dtype), torch.tensor(float("nan"))
    )
    masked_tensor1 = torch.where(
        neighbor_mask_1, neighbor_mask_1.to(dtype), torch.tensor(float("nan"))
    )
    ...
    if current_batch_size == 1:
        ...
        dists = sequential_sinkhorn_computations(
            batch_neighborhood_pc0, batch_neighborhood_pc1, params_sinkhorn
        )
        return dists
    ...
    max_neighbors_size = max(
        neighbors_per_batch0.max().item(), neighbors_per_batch1.max().item()
    )
    ...
    if computation_size > COMPUTATION_THRESHOLD:
        ...
        for nm_0, nm_1 in zip(neighbor_mask_batches_0, neighbor_mask_batches_1):
            ...
            dists[ind : ind + small_batch_size] = sinkhorn_divergence(
                PC_pair, nm_0, nm_1, params_sinkhorn
            )
    ...
    sorted_indices_pc0 = (
        not_nan_mask_pc0.cpu().sort(dim=1, descending=True).indices.to(device)
    )
    sorted_indices_pc1 = (
        not_nan_mask_pc1.cpu().sort(dim=1, descending=True).indices.to(device)
    )
    del not_nan_mask_pc0, not_nan_mask_pc1

    # Use the sorted indices to gather the points in the desired sorted order
    sorted_pc0 = torch.gather(
        batch_neighborhood_pc0, 1, sorted_indices_pc0.unsqueeze(2).expand(-1, -1, 3)
    )
    sorted_pc1 = torch.gather(
        batch_neighborhood_pc1, 1, sorted_indices_pc1.unsqueeze(2).expand(-1, -1, 3)
    )
    ...
    if not valid_batches.any().item():
        batch_distances = torch.full(
            (current_batch_size,), params_sinkhorn.max_dist, dtype=dtype, device=device
        )
    ...
    loss_fn = SamplesLoss(
        loss="sinkhorn", p=params_sinkhorn.p, blur=params_sinkhorn.blur
    )
    ...
    batch_distances = torch.full(
        (current_batch_size,), max_distance, dtype=dtype, device=device
    )
...
def sequential_sinkhorn_computations(
    batch_neighborhood_pc0, batch_neighborhood_pc1, params_sinkhorn
):
